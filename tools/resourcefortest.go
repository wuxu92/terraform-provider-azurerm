...
const ConstB0 = "const b0"

const ConstB1 = "const b1"

type StrTye string

const (
	StrTypeA0 = "str type a0"
	StrTypeA1 = "str type a1"
)

func PossibleStrTypeValues() []string {
	return []string{
		string(ConstB0),
		string(ConstB1),
		ConstA0,
...
func PossibleStrTypeFunc() []StrTye {
	return []StrTye{
		StrTypeA0,
		StrTypeA1,
	}
				...
				Schema: map[string]*pluginsdk.Schema{
					...
					"subnet_id": {
						...
						ValidateFunc: validation.StringInSlice([]string{
							string(nginxdeployment.ProvisioningStateCanceled),
							string(nginxdeployment.ProvisioningStateSucceeded),
						}, false),
					},
					...
					"automation_str": {
						...
						Required: true,
						ValidateFunc: validation.StringInSlice(func() (res []string) {
							for _, val := range automation.PossibleAccountStateValues() {
			...
			Elem: &pluginsdk.Resource{
				Schema: map[string]*pluginsdk.Schema{
					"subnet_id": {
						Type:         pluginsdk.TypeString,
						Required:     true,
						ValidateFunc: validation.StringInSlice(possibleForTest(), false),
					},
				},
			},
		},
