...
* `transparent_data_encryption_managed_hsm_key_id` - The Managed HSM key URI to be used as the `Customer Managed Key`(CMK/BYOK) for the `Transparent Data Encryption`(TDE) layer.
