...
func TestAccMsSqlDatabase_transparentDataEncryptionManagedHSMKey(t *testing.T) {
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.transparentDataEncryptionManagedHSMKey(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func (r MsSqlDatabaseResource) transparentDataEncryptionManagedHSMKey(data acceptance.TestData) string {
	return fmt.Sprintf(`
%s
...
resource "azurerm_mssql_database" "test" {
  ...
  sku_name                                                   = "S0"
  transparent_data_encryption_enabled                        = true
  transparent_data_encryption_managed_hsm_key_id             = azurerm_key_vault_managed_hardware_security_module_key.test.versioned_id
  transparent_data_encryption_key_automatic_rotation_enabled = true
  ...
  depends_on = [
    azurerm_key_vault_managed_hardware_security_module_role_assignment.identity,
  ]
}
`, r.template(data), data.RandomInteger, data.RandomString)
}
