...
func resourceKeyVaultManagedHardwareSecurityModuleKey() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		Create: resourceKeyVaultManagedHardwareSecurityModuleKeyCreate,
		Read:   resourceKeyVaultManagedHardwareSecurityModuleKeyRead,
		Update: resourceKeyVaultManagedHardwareSecurityModuleKeyUpdate,
		Delete: resourceKeyVaultManagedHardwareSecurityModuleKeyDelete,

		Importer: pluginsdk.ImporterValidatingResourceIdThen(func(id string) error {
			_, err := parse.ParseNestedItemID(id)
			return err
		}, nestedItemManagedHSMResourceImporter),
...
func resourceKeyVaultManagedHardwareSecurityModuleKeyCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	keyVaultsClient := meta.(*clients.Client).KeyVault
	client := meta.(*clients.Client).KeyVault.ManagedHSMManagementClient
	ctx, cancel := timeouts.ForCreate(meta.(*clients.Client).StopContext, d)
	defer cancel()

	log.Print("[INFO] preparing arguments for AzureRM KeyVault Key creation.")
	name := d.Get("name").(string)
	keyVaultId, err := managedhsms.ParseManagedHSMID(d.Get("managed_hsm_id").(string))
	if err != nil {
		return err
	}

	managedHSMBaseUri, err := keyVaultsClient.BaseUriForManagedHSM(ctx, *keyVaultId)
	if err != nil {
	...
	if err != nil {
		if !utils.ResponseWasNotFound(existing.Response) {
			return fmt.Errorf("checking for presence of existing Key %q (Key Vault %q): %s", name, *managedHSMBaseUri, err)
		}
	}
	...
	keyOptions := expandManagedHSMKeyOptions(d)
	...
	if v, ok := d.GetOk("rotation_policy"); ok {
		if respPolicy, err := client.UpdateKeyRotationPolicy(ctx, *managedHSMBaseUri, name, expandKeyVaultKeyRotationPolicy(v.([]interface{}))); err != nil {
			if utils.ResponseWasForbidden(respPolicy.Response) {
				return fmt.Errorf("current client lacks permissions to create Key Rotation Policy for Key %q (%q, Vault url: %q), please update this as described here: %s : %v", name, *keyVaultId, *managedHSMBaseUri, "https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/key_vault_key#example-usage", err)
			}
	...
	if read.Key == nil || read.Key.Kid == nil {
		return fmt.Errorf("cannot read KeyVault Key '%s' (in key vault '%s')", name, *managedHSMBaseUri)
	}
	...
	return resourceKeyVaultManagedHardwareSecurityModuleKeyRead(d, meta)
}

func resourceKeyVaultManagedHardwareSecurityModuleKeyUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	keyVaultsClient := meta.(*clients.Client).KeyVault
	client := meta.(*clients.Client).KeyVault.ManagedHSMManagementClient
	...
	managedHSMId, err := managedhsms.ParseManagedHSMID(d.Get("managed_hsm_id").(string))
	if err != nil {
		return err
	}

	meta.(*clients.Client).KeyVault.AddManagedHSMToCache(*managedHSMId, id.KeyVaultBaseUrl)

	ok, err := keyVaultsClient.ManagedHSMExists(ctx, *managedHSMId)
	if err != nil {
		return fmt.Errorf("checking if key vault %q for Key %q in Vault at url %q exists: %v", *managedHSMId, id.Name, id.KeyVaultBaseUrl, err)
	}
	if !ok {
		log.Printf("[DEBUG] Key %q Key Vault %q was not found in Key Vault at URI %q - removing from state", id.Name, *managedHSMId, id.KeyVaultBaseUrl)
		d.SetId("")
		return nil
	}
	...
	if d.HasChange("rotation_policy"); ok {
		if respPolicy, err := client.UpdateKeyRotationPolicy(ctx, id.KeyVaultBaseUrl, id.Name, expandKeyVaultKeyRotationPolicy(d.Get("rotation_policy").([]interface{}))); err != nil {
			if utils.ResponseWasForbidden(respPolicy.Response) {
				return fmt.Errorf("current client lacks permissions to update Key Rotation Policy for Key %q (%q, Vault url: %q), please update this as described here: %s : %v", id.Name, *managedHSMId, id.KeyVaultBaseUrl, "https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/key_vault_key#example-usage", err)
			}
			return fmt.Errorf("creating Key Rotation Policy: %+v", err)
		}
	}
...
func resourceKeyVaultManagedHardwareSecurityModuleKeyRead(d *pluginsdk.ResourceData, meta interface{}) error {
	keyVaultsClient := meta.(*clients.Client).KeyVault
	client := meta.(*clients.Client).KeyVault.ManagedHSMManagementClient
	...
	managedHSMIDRaw, err := keyVaultsClient.ManagedHSMIDFromBaseUrl(ctx, subscriptionResourceId, id.KeyVaultBaseUrl)
	if err != nil {
		return fmt.Errorf("retrieving the Resource ID the Key Vault at URL %q: %s", id.KeyVaultBaseUrl, err)
	}
	if managedHSMIDRaw == nil {
	...
	d.Set("resource_id", parse.NewKeyID(managedHSMId.SubscriptionId, managedHSMId.ResourceGroupName, managedHSMId.ManagedHSMName, id.Name, id.Version).ID())
	d.Set("resource_versionless_id", parse.NewKeyVersionlessID(managedHSMId.SubscriptionId, managedHSMId.ResourceGroupName, managedHSMId.ManagedHSMName, id.Name).ID())
	...
	if err != nil {
		...
		case utils.ResponseWasForbidden(respPolicy.Response):
			...
			return fmt.Errorf("current client lacks permissions to read Key Rotation Policy for Key %q (%q, Vault url: %q), please update this as described here: %s : %v", id.Name, *managedHSMId, id.KeyVaultBaseUrl, "https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/key_vault_key#example-usage", err)
...
func resourceKeyVaultManagedHardwareSecurityModuleKeyDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	keyVaultsClient := meta.(*clients.Client).KeyVault
	client := meta.(*clients.Client).KeyVault.ManagedHSMManagementClient
	subscriptionId := meta.(*clients.Client).Account.SubscriptionId
	...
	kv, err := keyVaultsClient.ManagedHsmClient.Get(ctx, *managedHSMId)
	if err != nil {
		...
		return fmt.Errorf("retrieving key vault %q properties: %+v", *managedHSMId, err)
	}
...
func expandManagedHSMKeyOptions(d *pluginsdk.ResourceData) *[]keyvault.JSONWebKeyOperation {
	options := d.Get("key_opts").(*pluginsdk.Set).List()
