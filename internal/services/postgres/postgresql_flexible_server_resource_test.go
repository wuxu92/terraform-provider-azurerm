...
func TestAccPostgresqlFlexibleServer_createWithCustomerManagedHSMKey(t *testing.T) {
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.withCustomerManagedHSMKey(data),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That("azurerm_postgresql_flexible_server.test").Key("customer_managed_key.0.managed_hsm_key_id").Exists(),
				check.That("azurerm_postgresql_flexible_server.test").Key("customer_managed_key.0.primary_user_assigned_identity_id").Exists(),
			),
		},
		data.ImportStep("administrator_password", "create_mode"),
...
func (r PostgresqlFlexibleServerResource) withCustomerManagedHSMKeyTemplate(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {
    key_vault {
...
func (r PostgresqlFlexibleServerResource) withCustomerManagedHSMKey(data acceptance.TestData) string {
	return fmt.Sprintf(`
%[1]s
...
`, r.withCustomerManagedHSMKeyTemplate(data), data.RandomInteger)
}
