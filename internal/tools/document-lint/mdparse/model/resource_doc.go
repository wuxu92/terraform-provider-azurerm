...
func (p Properties) Merge(p1 Properties) {
	for k, v := range p1 {
		...
		if f, ok := p[k]; ok {
			f.SameNameAttr = v
			//f.Required = f.Required | v.Required
		} else {
			p[k] = v
		}
	}
...
type Field struct {
	...
	TFRequied bool `json:",omitempty"` // requriedness in golang code
...
func (r *ResourceDoc) AllProp() Properties {
	...
	for k, v := range r.Attr {
		...
		if arg, ok := res[k]; ok && arg.Subs != nil {
...
func (r *ResourceDoc) ToJSON(indent bool) string {
	var bs []byte
	if indent {
